...
class NLP:
    ...
    def question_answering(self, prompt):
...
class Note_App:
    ...
    def edit_note(self, title):
        # get the note
        self.note = self.database_manager.get_single_note(title)
        self.edit_window = tk.Tk()
        self.edit_window.title("Edit Note")
        self.screen_width = self.edit_window.winfo_screenwidth()
        self.screen_height = self.edit_window.winfo_screenheight()
        self.edit_window.geometry(f"{int(self.screen_width*0.9)}x{int(self.creen_height*0.9)}")
        
        # create the title entry
        title_entry = tk.Entry(self.edit_window, width=50)
        title_entry.grid(row=0, column=0, padx=50, pady=50)
        title_entry.insert(0, self.note[0])
        # create the text entry
        text_entry = tk.Text(self.edit_window, width=50, height=30)
        text_entry.grid(row=1, column=0, padx=50, pady=50)
        text_entry.insert(tk.END, self.note[1])
        # create a entry for the date
        date_entry = tk.Entry(self.edit_window, width=50)
        date_entry.grid(row=2, column=0, padx=50, pady=50)
        date_entry.insert(0, self.note[2])
        # create the tags entry
        tags_entry = tk.Entry(self.edit_window, width=50)
        tags_entry.grid(row=2, column=0, padx=50, pady=50)
        tags_entry.insert(0, self.note[3])
        # create the save button
        save_button = tk.Button(self.edit_window, text="Save", command=lambda: save_note(title))
        ...
        def get_wiki_summary(prompt,last_word):
            ...
            if "+" in word:
                ...
                print(chapters)
                # create a new window
                wiki_window = tk.Toplevel(self.edit_window)
                wiki_window.title("Wikipedia")
                wiki_window.geometry("350x350")
                # add text to textarea
                text_entry.insert(tk.END, text)
        ...
        def question_answering(prompt):
            ...
            # create a new window
            qa_window = tk.Toplevel(self.edit_window)
            qa_window.title("Question Answering")
            qa_window.geometry("350x350")
            # question entry
            question_entry = tk.Text(qa_window, height=10, width=40)
        ...
        def save_note(title):
            ...
            # update the note
            self.Database_Manager.update_single_note(new_title, new_text, new_date, new_tags)
            self.edit_window.destroy()
            self.get_all_notes()

    def open_notes(self, title):
        # get the note from the database
        note = self.Database_Manager.get_single_note(title)
        ...
        # buttons
        delete_button = tk.Button(note_window, text="Delete", command=lambda: self.delete_note(title))
        edit_button = tk.Button(note_window, text="Edit", command=lambda: self.edit_note(title))
    ...
    def all_notes(self):
        ...
        for _, note in enumerate(notes):
            ...
            def onselect(evt):
                ...
                edit_button = tk.Button(self.window, text="Edit", command=lambda: self.edit_note(title))
                # place it in the grid
                edit_button.grid(row=0, column=1, padx=10, pady=10)
                delete_button = tk.Button(self.window, text="Delete", command=lambda: self.delete_note(title))
                # place it in the grid
